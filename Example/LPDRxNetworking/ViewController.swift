...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
      ...
      request1.path = "http://baike.baidu.com/api/openapi/BaikeLemmaCardApi"
    ...
    //  disposable?.dispose()
      ...
      let batchRequest:LPDBatchRequest = LPDBatchRequest()
      batchRequest.addRequest(request: request1)
      batchRequest.addRequest(request: request2)
      
      batchRequest.rx_value()?.subscribe(onNext: { (x) in
        print(x)
      }, onError: { (error) in
        print("error--")
        print(error)
